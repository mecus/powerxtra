...
@Component({
  ...
  imports: [
   ...
   templateUrl: './popup-library.html',
  styleUrl: './popup-library.scss',
})
export class PopupLibrary implements OnInit {
  librarySise = 0;
  tracks: ITrack[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PopupLibrary>,
  ...
  async addToPlaylist() {
    try{
      ...
      const data: any = this.mapTracks(tracks);
      console.log(data)
      this.appService.startSpinner(`Adding ${tracks.length} Tracks to Album...`);
      const res = await this.apiService.addTracksToAlbum({list: data});
      if(res) {
